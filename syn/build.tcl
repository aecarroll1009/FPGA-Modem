...
#
# Four tops are buildable: rx_top (FPGA RX chain), tx_top (FPGA TX chain),
# tt_um_cordic_ddc (the taped-out unit, mixing direction as a live pin), and
# DE1_SoC (the pinned-out board build; see RESERVE_ALL_UNUSED_PINS below).
#
# Output lands in syn/output/ (gitignored); the Quartus project is generated
# from this script rather than checked in.

load_package flow

set top    [expr {[llength $argv] > 0 ? [lindex $argv 0] : "rx_top"}]
# Cyclone V SE A5, speed grade 6 -- the DE1-SoC part. The board's ordering
# code is 5CSEMA5F31C6N; Quartus rejects the trailing N ("Part name is
# invalid"), so the device name drops it. Override via the second argument.
